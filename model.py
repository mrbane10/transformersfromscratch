...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float):
        ...
        # Calculating denominator in log space for numerical stability
        denominator = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))
        # Apply sin for even positions
        pe[:, 0::2] = torch.sin(position * denominator)
        # Apply cos for odd positions
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        return (attention_scores @ value), attention_scores  # for visualisation
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, src_mask):   
        x = self.residual_connections[0](x, lambda x: self.self_attention_block(x, x, x, src_mask)) # to hide the padding words we apply the mask
        x = self.residual_connections[1](x, self.feed_forward_block)
        return x 

class Encoder(nn.Module): # now defining the encoder objects i.e combining our enocder blocks
...
class Transformer(nn.Module):
    def __init__(self, encoder: Encoder, decoder: Decoder, src_embedding: InputEmbedding, 
                 target_embedding: InputEmbedding, src_pos: PositionalEncoding, 
...
def build_transformer(src_vocab_size: int, target_vocab_size: int, src_seq_len: int, 
                     target_seq_len: int, d_model: int = 512, N: int = 6, 
